...
/**
 * Operations for building minimal deterministic automata from sets of strings. The algorithm requires sorted
 * input data, but is very fast (nearly linear with the input size).
 * 
 * @author Dawid Weiss
 */
final public class StringUnionOperations {
	...
	final static class State {
		...
		/**
		 * Returns the target state of a transition leaving this state and labeled with <code>label</code>. If
		 * no such transition exists, returns <code>null</code>.
		 */
		...
		/**
		 * Returns an array of outgoing transition labels. The array is sorted in lexicographic order and
		 * indexes correspond to states returned from {@link #getStates()}.
		 */
		 ...
		 * <ul>
		 * <li>they have an identical number of outgoing transitions, labeled with the same labels</li>
		...
		public int hashCode() {
			...
			/*
			 * Compare the right-language of this state using reference-identity of outgoing states. This is
			 * possible because states are interned (stored in registry) and traversed in post-order, so any
			 * outgoing transitions are already interned.
			 */
		...
		/**
		 * Create a new outgoing transition labeled <code>label</code> and return the newly created target
	...
	/**
	 * Add another character sequence to this automaton. The sequence must be lexicographically larger or
	 * equal compared to any previous sequences added to this automaton (the input must be sorted).
	 */
	...
	/**
	 * Finalize the automaton and return the root state. No more strings can be added to the builder after
	 * this call.
	 * 
	...
	/**
	 * Replace last child of <code>state</code> with an already registered state or register the last child
	 * state.
	 */
