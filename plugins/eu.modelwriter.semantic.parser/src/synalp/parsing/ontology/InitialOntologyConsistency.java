...
/**
 * This class was used once to run experiments with the initial Airbus ontology, so the code is not optimised.
 * It loads the original inconsistent ontology and builds a new ontology by adding axioms of each type
 * incrementally. That allows us to see at which step the reasoner complains about inconsistency. It has
 * separate loops for each type of axioms so that they can be easily reordered and modified. We had to make
 * different combinations of the order in which axioms are loaded to the new ontology as, in some
 * combinations, the reasoner is heavily affected (it slows down or runs out of memory).
 * 
 * @author Anastasia
 */
...
public class InitialOntologyConsistency {
	...
	public static void main(String[] args) throws Exception {
		...
		for (OWLAxiom axiom : axioms) {
			if (axiom.isOfType(AxiomType.DATA_PROPERTY_ASSERTION) || axiom.isOfType(
		...
		for (OWLAxiom axiom : axioms) {
			if (axiom.isOfType(AxiomType.SUB_DATA_PROPERTY) || axiom.isOfType(
					AxiomType.FUNCTIONAL_DATA_PROPERTY) || axiom.isOfType(AxiomType.DATA_PROPERTY_DOMAIN)
		...
		for (OWLAxiom axiom : axioms) {
			if (axiom.isOfType(AxiomType.SUB_OBJECT_PROPERTY) || axiom.isOfType(
					AxiomType.INVERSE_OBJECT_PROPERTIES) || axiom.isOfType(
							AxiomType.FUNCTIONAL_OBJECT_PROPERTY) || axiom.isOfType(
									AxiomType.INVERSE_FUNCTIONAL_OBJECT_PROPERTY) || axiom.isOfType(
											AxiomType.TRANSITIVE_OBJECT_PROPERTY) || axiom.isOfType(
													AxiomType.SYMMETRIC_OBJECT_PROPERTY) || axiom.isOfType(
		...
		// Annotation Axioms
		for (OWLAxiom axiom : axioms) {
			if (axiom.isOfType(AxiomType.ANNOTATION_ASSERTION) || axiom.isOfType(
					AxiomType.ANNOTATION_PROPERTY_DOMAIN) || axiom.isOfType(
		...
		for (OWLAxiom axiom : axioms) {
			if (axiom.isOfType(AxiomType.DATATYPE_DEFINITION) || axiom.isOfType(
		...
		// Save ontology
		manager.saveOntology(ontology, newOntologyIRI);
...
/*
 * NodeSet<OWLNamedIndividual> individuals = reasoner.getInstances(cls, false); // get indirect individuals,
 * which belong to subclasses if(individuals.isEmpty()) { if (!reasoner.isSatisfiable(cls)) {
 * log.log(Level.INFO, "it is an unsatisfiable class: " + cls); } else { log.log(Level.INFO,
 * "it is a satisfiable class: " + cls); } IRI individIRI = IRI.create(cls.getIRI() + "1"); OWLIndividual
 * newIndivid = dataFactory.getOWLNamedIndividual(individIRI); addClassAssertionAxiom(cls, newIndivid); } else
...
/*
 * System.out.println("Computing explanations for the inconsistency..."); BlackBoxExplanation exp = new
 * BlackBoxExplanation(ontology, factory, reasoner); HSTExplanationGenerator multExplanator = new
 * HSTExplanationGenerator(exp); // Now we can get explanations for the inconsistency Set<Set<OWLAxiom>>
 * explanations = multExplanator.getExplanations(dataFactory.getOWLThing()); // Let us print them. Each
 * explanation is one possible set of axioms that cause the // unsatisfiability. for (Set<OWLAxiom>
 * explanation : explanations) { System.out.println("------------------");
 * System.out.println("Axioms causing the inconsistency: "); for (OWLAxiom causingAxiom : explanation) {
